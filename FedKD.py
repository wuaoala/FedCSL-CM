...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            ...
            if iter == 0:
                w, loss = local.fedavg_train(
                    net=copy.deepcopy(net_local).to(args.device))
            else:
                net_local.load_state_dict(torch.load('./fedkd_model_param/local_net_{}.pkl'.format(idx)))
                w, loss = local.kd_train(local_net=copy.deepcopy(net_local).to(args.device),
                    kd_net=copy.deepcopy(kd_net).to(args.device))
            w_locals.append(copy.deepcopy(w.state_dict()))
            torch.save(w.state_dict(), './fedkd_model_param/local_net_{}.pkl'.format(idx))
            # 保存这一轮参与聚合的局部模型的训练损失
    ...
    # print('FedKD算法 Average Global AUC-ROC: {:.4f}'.format(sum(fedavg_test_AUC_ROC) / args.epochs))
    # print('FedKD算法 Average Global AUC-PR: {:.4f}'.format(sum(fedavg_test_AUC_PR) / args.epochs))
    # print('FedKD算法 Average Global KS: {:.4f}'.format(sum(fedavg_test_KS) / args.epochs))
    # print('FedKD算法 Average Global BS+: {:.4f}'.format(sum(fedavg_test_BS_plus) / args.epochs))
    # avg_epoch_time = sum(avg_epoch_time) / len(avg_epoch_time)
    # print('FedKD算法 Average Epoch Time: {:.4f}s'.format(avg_epoch_time))
    ...
    # list2txt(result, "./save/{} FedKD results.txt".format(args.dataset))
