...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            ...
            if iter == 0:
                ...
                w, loss = local.fedprox_train(net=copy.deepcopy(net_glob).to(args.device),
                                              glob_net=copy.deepcopy(net_glob).to(args.device))
    ...
    # print('FedProx算法 Average Global AUC-ROC: {:.4f}'.format(sum(fedavg_test_AUC_ROC) / args.epochs))
    # print('FedProx算法 Average Global AUC-PR: {:.4f}'.format(sum(fedavg_test_AUC_PR) / args.epochs))
    # print('FedProx算法 Average Global KS: {:.4f}'.format(sum(fedavg_test_KS) / args.epochs))
    # print('FedProx算法 Average Global BS+: {:.4f}'.format(sum(fedavg_test_BS_plus) / args.epochs))
    # avg_epoch_time = sum(avg_epoch_time) / len(avg_epoch_time)
    # print('FedProx算法 Average Epoch Time: {:.4f}s'.format(avg_epoch_time))
    ...
    # list2txt(result, "./save/{} FedProx results.txt".format(args.dataset))
