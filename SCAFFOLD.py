...
from models.Fed import FedAvg,SCAFFOLD,FedNova
import time
...
if __name__ == '__main__':
    ...
    fedavg_train_loss = []
    avg_epoch_time = []
    C_g = [torch.zeros_like(param).to(args.device) for param in net_glob.parameters()]
    C_l = copy.deepcopy(C_g)
    avg_epoch_time = []
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            ...
            if iter == 0:
                C_plus, w, loss = local.SCAFFOLD_train(net=copy.deepcopy(net_glob).to(args.device),
                                                       c_global=C_g, c_local=C_l)
            else:
                C_plus, w, loss = local.SCAFFOLD_train(net=copy.deepcopy(net_glob).to(args.device),
                                                       c_global=C_g, c_local=C_l[idx])
        ...
        # for idx in np.arange(args.num_users):
            local = LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users[idx])
            glob_test_gmean, glob_test_ba, glob_test_f1_score, glob_test_recall, profit, cost = local.test(net=copy.deepcopy(net_glob))
    ...
    print('SCAFFOLD算法 Average Global AUC-ROC: {:.4f}'.format(sum(fedavg_test_AUC_ROC) / args.epochs))
    print('SCAFFOLD算法 Average Global AUC-PR: {:.4f}'.format(sum(fedavg_test_AUC_PR) / args.epochs))
    print('SCAFFOLD算法 Average Global KS: {:.4f}'.format(sum(fedavg_test_KS) / args.epochs))
    print('SCAFFOLD算法 Average Global BS+: {:.4f}'.format(sum(fedavg_test_BS_plus) / args.epochs))
    avg_epoch_time = sum(avg_epoch_time) / len(avg_epoch_time)
    print('SCAFFOLD算法 Average Epoch Time: {:.4f}s'.format(avg_epoch_time))
    ...
    list2txt(result, "./save/{} SCAFFOLD results.txt".format(args.dataset))
